...
export async function findAll(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const announcements: IAnnouncementsModel[] = await AnnouncementsService.findAll(pageNo, pageSize);
        const totalCount = await AnnouncementsService.countAll();
        res.status(200).json({ totalCount: totalCount, records: announcements, pageNo: pageNo + 1, pageSize: pageSize });
    } catch (error) {
...
export async function search(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const pageNo = req.query.pageNo ? parseInt(req.query.pageNo.toString()) - 1 : 0;
        const pageSize = req.query.pageSize ? parseInt(req.query.pageSize.toString()) : 20;
        const announcements: IAnnouncementsModel[] = await AnnouncementsService.search(req.body, pageNo, pageSize);
        const totalCount = await AnnouncementsService.countSearch(req.body);
        res.status(201).json({ totalCount: totalCount, records: announcements, pageNo: pageNo, pageSize: pageSize });
    } catch (error) {
        next(new HttpError(error.message.status, error.message));
    }
}
...
export async function findOne(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const announcements: IAnnouncementsModel = await AnnouncementsService.findOne(req.params.id);
...
export async function create(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const announcements: IAnnouncementsModel = await AnnouncementsService.insert(req.body);
...
export async function update(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const announcements = await AnnouncementsService.update(req.params.id, req.body);
        if (announcements > 0) {
...
export async function remove(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const announcements: IAnnouncementsModel = await AnnouncementsService.remove(req.params.id);
